function ecef = eci2ecef(eci, utc)
    % Transform a position vector from the ECI coordinate system to the ECEF coordinate system
    % 
    % Inputs:
    %   eci: position vector in ECI (3 x 1)
    %   utc: UTC of transformation (scalar, datetime)
    %
    % Output:
    %   ecef: position vector in ECEF (3 x 1)

    timehack = [year(utc), month(utc), day(utc), hour(utc), minute(utc), second(utc)];
    gast = siderealtime(timehack);
    dcm = CoordinateRotation("z", gast);
    ecef = dcm * eci;
end
